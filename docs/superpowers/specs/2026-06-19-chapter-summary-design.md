...
1. 阅读页 `设置` 面板新增 `AI 本章梗概` 小节。
   - 普通用户看到 `自动生成本章梗概` 开关和短说明。
   - 管理员额外看到 `高级配置`：功能启用、默认自动、prompt、详细程度、最大字数、temperature、最短正文长度。
2. 桌面右侧栏新增一个 `摘要` 快捷按钮。
   - 这个按钮只切换当前用户的 `自动生成本章梗概` on/off。
   - 开启状态用主色高亮；关闭状态保持普通透明态。
   - 点击后 toast 提示 `已开启自动摘要` 或 `已关闭自动摘要`。
   - 不打开抽屉，不进入配置页，避免和 `设置` 重复。

移动端先不新增单独按钮，继续通过设置面板控制；摘要卡片本身仍可手动生成。
...
4. ReadSettings 增加自动摘要开关和管理员高级配置入口。
5. ReaderSidebar 增加 `摘要` 自动开关快捷按钮。
6. 测试和本地 smoke。
